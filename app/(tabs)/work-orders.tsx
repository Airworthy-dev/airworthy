  ...
  return (
    <SafeAreaView style={s.safeArea}>
      ...
      <View style={s.searchContainer}>
        <TextInput style={s.searchInput} placeholder="Search by ID, tail, or description…" placeholderTextColor={colors.subtext} value={search} onChangeText={setSearch} />
      </View>

      <ScrollView horizontal showsHorizontalScrollIndicator={false} style={s.filterRow} contentContainerStyle={s.filterContent}>
        {FILTERS.map((f) => (
          <TouchableOpacity key={f.value} style={[s.filterChip, filter === f.value && s.filterChipActive]} onPress={() => setFilter(f.value)}>
      ...
      <ScrollView style={s.list} contentContainerStyle={s.listContent} showsVerticalScrollIndicator={false}>
        {filtered.length === 0 ? (
          <View style={s.empty}><Text style={s.emptyText}>No work orders found</Text></View>
        ) : (
          filtered.map((wo) => {
